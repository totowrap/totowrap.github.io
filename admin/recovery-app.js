...
const db = null;
const auth = null;
const STATE_REF = null;
const RECOVERY_STORAGE_KEY = 'totowrap-recovery-site-state-v1';
async function signInWithEmailAndPassword() { return { user: { email: 'recovery@local' } }; }
async function signOut() { return true; }
function getDocFromServer() { return Promise.resolve({ exists: () => false, data: () => ({}) }); }
async function runTransaction(_db, callback) {
  let written = null;
  const tx = {
    async get() { return { exists: () => true, data: () => S }; },
    set(_ref, nextState) { written = nextState; }
  };
  await callback(tx);
  if (written) localStorage.setItem(RECOVERY_STORAGE_KEY, JSON.stringify(written));
}
function onAuthStateChanged(_auth, callback) { callback({ email: 'recovery@local' }); }
...
async function saveS() {
  ...
  nextState._recoveryMeta = {
    mode: 'local-recovery-copy',
    savedAt: new Date().toISOString(),
    note: 'Created by admin/recovery-site.html. This state has not been written to Firestore.'
  };
  S = nextState;
  localStorage.setItem(RECOVERY_STORAGE_KEY, JSON.stringify(S));
  recoveryPublishState();
  return true;
}
...
function chatResultBeforeStopForDay(messages, stop, dayNumber) {
  if (!stop || !Number.isFinite(dayNumber)) return null;
  return [...messages]
    .filter(msg =>
      (msg.dt < stop.dt || (msg.dt.getTime() === stop.dt.getTime() && msg.index < stop.index))
      && isLuigiVacchelli(msg.sender)
      && chatBodyIsResultBoundary(msg.body)
      && chatResultDay(msg.body) === dayNumber
    )
    .sort((a, b) => b.dt - a.dt || b.index - a.index)[0] || null;
}

function chatStopsForDate(messages, targetISO) {
  return [...messages]
...
function chatMessageInsideRecoveryWindow(msg, start, stop) {
  if (!stop) return false;
  const afterStart = !start || msg.dt > start.dt || (msg.dt.getTime() === start.dt.getTime() && msg.index > start.index);
  const beforeStop = msg.dt < stop.dt || (msg.dt.getTime() === stop.dt.getTime() && msg.index < stop.index);
  return afterStart && beforeStop;
}
...
function extractRecoveryBetsFromChatText(chatText, targetISO, targetDayNumber) {
  const messages = parseWhatsAppMessages(chatText);
  if (!messages.length) throw new Error('No WhatsApp messages found in the uploaded file.');
  const duplicateFirstNames = duplicateChatFirstNames(messages);
  const ignoredIndexes = chatLuigiAnnouncementIndexes(messages);
  const stops = chatStopsForDate(messages, targetISO);
  if (!stops.length) throw new Error(`No Luigi Stop message found on ${displayDate(targetISO)}.`);

  let stop = stops[0];
  let start = null;
  const previousDayNumber = targetDayNumber - 1;
  if (targetDayNumber > 0) {
    const matchingStop = stops.find(candidate => chatResultBeforeStopForDay(messages, candidate, previousDayNumber));
    if (!matchingStop) throw new Error(`No "TotoWrap result - Day ${previousDayNumber}" boundary found before Luigi's Stop on ${displayDate(targetISO)}.`);
    stop = matchingStop;
    start = chatResultBeforeStopForDay(messages, stop, previousDayNumber);
  }

  const bets = new Map();
  [...messages].sort((a, b) => a.dt - b.dt || a.index - b.index).forEach(msg => {
    if (ignoredIndexes.has(msg.index)) return;
    if (!chatMessageInsideRecoveryWindow(msg, start, stop)) return;
    if (chatBodyIsRecapList(msg.body)) return;
    extractChatBetsFromBody(msg.body, msg.sender, duplicateFirstNames).forEach(([name, betTime]) => {
      bets.set(name, betTime);
    });
  ...
  return {
    text,
    count: bets.size,
    stopTime: stop.timeText,
    stopDate: chatDateISO(stop.dt),
    boundaryDay: start ? chatResultDay(start.body) : null
  };
}

function recoveryPreviewApprovedAt() {
  const candidate = normalizeHMSInput(S.today?.approvedAt || '');
  return isValidHMS(candidate) ? candidate : nowHMS();
}
...
async function showPreview() {
  ...
  const previewApprovedAt = recoveryPreviewApprovedAt();
...
function bindMain() {
  ...
  document.getElementById('new-day-btn')?.addEventListener('click', async () => {
    const prevS = cloneState();
    const nextRecoveryDate = S.today?.date ? addDaysISO(S.today.date, 1) : localDateISO();
    if(S.today&&S.today.wrapTime) {
      S.days.push({...S.today});
      sortHistoryDaysByDate();
    }
    S.today={date:nextRecoveryDate,guesses:[],wrapTime:null,wrapDate:null,winner:null,points:null,estWrap:null,approvedAt:null,approvedDate:nextRecoveryDate};
    const saved = await saveS();
    if (!saved) { restoreAfterFailedSave(prevS); return; }
    render();
  });
...
function recoveryPublishState() {
  ...
  window.dispatchEvent(new CustomEvent('totowrap-recovery-state-changed', { detail: JSON.parse(JSON.stringify(S)) }));
}

function recoveryLoadState(nextState, { save=true }={}) {
  currentUser = { email: 'recovery@local' };
  authReady = true;
  ...
  if (save) localStorage.setItem(RECOVERY_STORAGE_KEY, JSON.stringify(S));
  recoveryPublishState();
  render();
}

function recoveryDownloadState() {
  const state = normalizeState(cloneState());
  state._recoveryMeta = {
    mode: 'local-recovery-copy',
    exportedAt: new Date().toISOString(),
    note: 'Review this JSON before uploading it to Firestore.'
  };
  const d = new Date();
  ...
  a.href = url;
  a.download = `totowrap_repaired_${stamp}.json`;
...
async function recoveryRecoverChatDay({ chatText, dayNumber, date }={}) {
  const parsedDay = Number(dayNumber);
  if (!Number.isInteger(parsedDay) || parsedDay < 0) {
    throw new Error('Enter a valid Day number, starting from 0.');
  }
  const targetISO = parseDateInput(date);
  if (!targetISO) {
    throw new Error('Enter a valid date as dd/mm/yyyy.');
  }
  if (!String(chatText || '').trim()) {
    throw new Error('Upload a WhatsApp _chat.txt first.');
  }
  if (S.today?.wrapTime) {
    throw new Error('Current recovery day is already wrapped. Click Start Next Day before recovering another day.');
  }
  if ((S.today?.guesses || []).some(g => g.time)) {
    throw new Error('Current recovery day already has started guesses. Finish or clear it before recovering another day.');
  }

  const result = extractRecoveryBetsFromChatText(chatText, targetISO, parsedDay);
  if (!result.text.trim()) {
    throw new Error(`No bets found for Day ${parsedDay} on ${displayDate(targetISO)}.`);
  }
  ...
  if (!S.today) {
    S.today = {
      date: targetISO,
      approvedDate: targetISO,
      approvedAt: result.stopTime,
      guesses: [],
      wrapTime: null,
      wrapDate: null,
      winner: null,
      ...
      noWinner: false,
      estWrap: null,
      estWrapDate: targetISO,
      betCloseAt: null,
      addedPlayers: []
    };
  }

  S.today.date = targetISO;
  S.today.approvedDate = targetISO;
  S.today.approvedAt = result.stopTime;
  S.today.estWrapDate = targetISO;
  _tab = 'today';
  const saved = await saveS();
  if (!saved) {
    restoreAfterFailedSave(prevS);
    throw new Error('Recovery state could not be saved locally.');
  }
  render();
  requestAnimationFrame(() => {
    const textarea = document.getElementById('paste-inp');
...
function recoveryResetState() {
  localStorage.removeItem(RECOVERY_STORAGE_KEY);
  recoveryLoadState({ playerRoster: [], scores: {}, days: [], today: null }, { save: false });
}

window.TotoWrapRecovery = {
  loadState: recoveryLoadState,
  recoverChatDay: recoveryRecoverChatDay,
  downloadState: recoveryDownloadState,
  resetState: recoveryResetState,
  getState: () => normalizeState(cloneState())
};

currentUser = { email: 'recovery@local' };
authReady = true;
const savedRecoveryState = localStorage.getItem(RECOVERY_STORAGE_KEY);
recoveryLoadState(savedRecoveryState ? JSON.parse(savedRecoveryState) : { playerRoster: [], scores: {}, days: [], today: null }, { save: false });
