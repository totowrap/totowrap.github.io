(() => {
  ...
  function drawStandingsNames(ctx, names, x, y, maxWidth, maxFontSize, minFontSize, lineGap) {
  ...
  function standingsImageLines(ctx, names, maxWidth) {
  ...
  async function renderFinalStandingsImage(data) {
    ...
    try {
      ...
      const podium = groups.filter(entry => entry.rank <= 3);
      ...
      if (!ctx) throw new Error('Could not create final standings image');
      ...
      const logo = await loadRecapImage('imgs/tonnowrapbig.png');
      if (logo) {
        const logoWidth = 860;
      ...
      podium.forEach(entry => {
        ...
        ctx.textAlign = 'left';
        drawStandingsNames(ctx, entry.players.map(player => player.name), left + 280, y + podiumHeight / 2, 1080, 96, 48, 8);
        ctx.font = "bold 42px 'Alte Haas Grotesk', sans-serif";
        ctx.fillText(`${entry.wins} ${word(entry.wins, 'GAME', 'GAMES')} WON`, left + 1540, y + podiumHeight / 2);
        ctx.font = "bold 82px 'Alte Haas Grotesk', sans-serif";
        ctx.fillText(`${entry.score} ${word(entry.score, 'PT', 'PTS')}`, left + 2050, y + podiumHeight / 2);
      ...
      columns.forEach((column, columnIdx) => {
        ...
        column.forEach((entry, rowIdx) => {
          ...
          ctx.textAlign = 'left';
          drawStandingsNames(ctx, [entry.name], x + 120, rowY + rowHeight / 2, 480, 52, 28, 4);
          ctx.font = "bold 27px 'Alte Haas Grotesk', sans-serif";
          ctx.fillText(`${entry.wins} ${word(entry.wins, 'GAME', 'GAMES')} WON`, x + 660, rowY + rowHeight / 2);
          ctx.fillStyle = yellow;
          ctx.font = "bold 47px 'Alte Haas Grotesk', sans-serif";
          ctx.fillText(`${entry.score} ${word(entry.score, 'PT', 'PTS')}`, x + 980, rowY + rowHeight / 2);
        });
      });
      ...
      if (zeroNames.length) {
        ...
        ctx.font = "bold 42px 'Alte Haas Grotesk', sans-serif";
        ctx.textAlign = 'center';
        standingsImageLines(ctx, zeroNames, contentWidth - 80).forEach(line => {
